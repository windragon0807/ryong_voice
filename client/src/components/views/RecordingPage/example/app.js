...
async function init() {
    ...
    const recorder = await setupRecordingWorkletNode(recordingOptions);
    const monitorNode = context.createGain();

    // We can pass this port across the app
    // and let components handle their relevant messages
    const visualizerCallback = setupVisualizers(recorder);
    const recordingCallback = handleRecording(recorder.port, recordingOptions);
    ...
    source.connect(recorder).connect(monitorNode).connect(context.destination);
}
 ...
 */
async function setupRecordingWorkletNode(recordingOptions) {
    ...
    const WorkletRecordingNode = new AudioWorkletNode(context, "recording-processor", {
        processorOptions: recordingOptions,
    });
 ...
 * @param {object} recordingOptions Microphone channel count,
 ...
 */
function handleRecording(processorPort, recordingOptions) {
    const recordButton = document.querySelector("#record");     // Start Recording 버튼
    const recordText = recordButton.querySelector("span");      // Start Recording 버튼의 "Start"
    const player = document.querySelector("#player");           // <Audio />
    const downloadButton = document.querySelector("#download"); // Download file
    ...
    const recordingEventCallback = async (event) => {
        ...
        if (event.data.message === "SHARE_RECORDING_BUFFER") {
            ...
            for (let i = 0; i < recordingOptions.numberOfChannels; i++) {
...
function setupMonitor(monitorNode) {
    ...
    const monitorButton = document.querySelector("#monitor"); // Turn monitor on Button
...
function setupVisualizers() {
    ...
    visToggle.addEventListener("click", (e) => {
        ...
        visToggle.querySelector("span").innerHTML = visualizationEnabled ? "Pause" : "Play";
    });
